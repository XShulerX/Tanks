...
namespace MVC
{
    public class AbilityFactory
    {
        ...
        public T Create<T>(BulletPool pool, AbilityModel abilityModel, Player player) where T: Ability
        {
            Ability ability = abilityModel.Element switch
            {
                ...
                Elements.Water => new WaterAbility(pool, abilityModel).SetPlayer(player),
                Elements.Terra => new TerraAbility(pool, abilityModel).SetPlayer(player).SetEnemies(_unitStorage),
